...
A tool that writes to both usually puts the actual diagnostic on stderr and the review prose on stdout; rendering both doubles the entry for the common case. The tail is captured at the same bound as stderr (`stderrTailBytes`), re-applied when the cause is rendered so a failure the model signalled — which carries its whole output — is held to it too, rendered as its last twenty lines, and marked with an `[earlier lines omitted]` line when the line bound cut it. The classifier's own line — the matched refusal, the bound a timeout expired, or, for a call with no exit status that was not cancelled, the error that stopped it — leads the detail unless the tail already holds it, since a refusal that exited zero has no other trace and a matched line may sit above the bound. A cancellation is excluded because `cancelled` in the summary is the whole diagnosis. When that leading line is the wrapped error, it is bounded and trimmed exactly as the tail is: a wrapped error carrying everything the tool said would otherwise flood the record the bound exists to keep small, and a differently normalised line would not match the tail it repeats. A failure no tool owns, such as a prompt that would not expand, is summarised by its own first line. The bound is a constant, not a setting: nothing in the spec asks for it to be tunable, and a setting nobody adjusts is a maintenance cost.

An executor that reports its failure on a channel of its own rather than on stderr — claude, in its result event — has that message appended to the captured stderr, so the stderr branch carries it and the stdout tail is left to the executors with no such channel. It is appended rather than substituted, so an unrelated runtime warning on stderr cannot displace it, and appended last, so it survives the capture bound when the stderr before it is long.
