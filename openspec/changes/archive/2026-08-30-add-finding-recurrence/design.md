...
Claude's `stream-json` carries tool-use blocks with structured input, and ties each line to the call that launched the sub-agent — but never to the agent's name, which `subagent_type` cannot carry because these definitions are passed as prompt text rather than registered agent types. So the claude preamble asks the executor to put the agent's name in the launch call's `description`, which rrev reads back; codex gets no such clause, because its format offers nothing to read it from. Codex's format does not line up the same way. Rather than specify a normalized subset that throws away what claude offers, or a contract codex cannot meet, the requirement is conditional: attribute to the agent where the format identifies it, fall back to the phase where it does not. The failure mode is a less informative display, never a wrong attribution.
...
- **Ledger crowds out the prompt.** A long run's ledger competes with the requirement checklist for prompt budget. → Truncate by raise-count, keeping the most-repeated, and say so in the prompt. Unlike the checklist's budget, which each expansion gets in full, `ledger_budget` bounds the finished prompt: a review prompt embedding seven reviewer agents expands `{{LEDGER}}` eight times, so the budget is divided across the sites the prompt actually holds and each copy announces its own cut.
