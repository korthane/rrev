...
- **One change to run behavior.** A round whose external tool produced nothing readable as a review no longer converges, so a quiet failure cannot be filed as a clean pass; the requirement below asks for exactly that. Otherwise every phase still terminates as it does today: this change alters what rrev records and displays, and what reviewers are told, not when a loop stops.
