...
- **WHEN** an iteration completes with no confirmed findings
- **THEN** the executor emits the review-done signal and the phase ends
...
Each review loop SHALL terminate on the first of: the convergence signal, its iteration limit, a detected stalemate, an unrecoverable executor failure, or a user break. rrev MUST report which condition ended the loop.
